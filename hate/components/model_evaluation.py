...
import numpy as np
import pandas as pd
from hate.logger import logger
...
class ModelEvaluation:
 ...
 data transformation artifact stage
    ...
    def get_best_model_from_gcloud(self) -> str:
        ...
        """
        try:
            logger.info("Entered the get_best_model_from_gcloud method of Model Evaluation class")
    ...
    def evaluate(self):
        ...
        try:
            ...
            logger.info(f"the confusion_matrix is {confusion_matrix(y_test,res)} ")
            return accuracy
        except Exception as e:
            raise CustomException(e, sys) from e
    ...
    def initiate_model_evaluation(self) -> ModelEvaluationArtifacts:
        ...
        try:
            ...
            logger.info("Loading currently trained model")
            ...
            logger.info("Fetch best model from gcloud storage")
            best_model_path = self.get_best_model_from_gcloud()

            logger.info("Check is best model present in the gcloud storage or not ?")
            if os.path.isfile(best_model_path) is False:
                is_model_accepted = True
                logger.info("glcoud storage model is false and currently trained model accepted is true")

            else:
                logger.info("Load best model fetched from gcloud storage")
                ...
                if best_model_accuracy > trained_model_accuracy:
                    is_model_accepted = True
                    logger.info("Trained model not accepted")
                else:
                    is_model_accepted = False
                    logger.info("Trained model accepted")
